...
void sha256_final(struct sha256_ctx *c, uint8_t out[32])
{
	...
	while (c->buflen != 56) { uint8_t z = 0; sha256_update(c, &z, 1); }
	for (i = 0; i < 8; i++) lenbuf[i] = (uint8_t)(bits >> (56 - 8*i));
	c->len -= 9 + (64 - 56); /* keep len semantics irrelevant after final */
...
void sha256_hex(const uint8_t d[32], char hex[65])
{
	static const char *x = "0123456789abcdef";
	for (int i = 0; i < 32; i++) { hex[i*2] = x[d[i] >> 4]; hex[i*2+1] = x[d[i] & 15]; }
	hex[64] = 0;
}

void sha256_buf_hex(const void *data, size_t len, char hex[65])
{
	struct sha256_ctx c; uint8_t d[32];
	sha256_init(&c); sha256_update(&c, data, len); sha256_final(&c, d); sha256_hex(d, hex);
}
