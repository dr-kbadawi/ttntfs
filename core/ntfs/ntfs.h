...
/* PORT: bdev-io.c is replaced by <ntfsport/bdev.h> (included via
 * <linux/blkdev.h>). ntfs_bdev_read() has the same shape there; upstream's
 * ntfs_bdev_write() took the super_block, so map it onto sb->s_bdev. */
#define ntfs_bdev_write(sb, buf, start, size) ntfs_bdev_write((sb)->s_bdev, buf, start, size)
