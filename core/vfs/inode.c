...
#include <linux/bio.h>	/* PORT: before ntfs.h (ntfs_bdev_write macro) */
...
struct inode *ntfs_iget(struct super_block *sb, u64 mft_no)
{
	...
	if (inode_state_read_once(vi) & I_NEW) {
		...
		if (unlikely(err)) {
			/*
			 * PORT: the kernel keeps a "bad" inode in the cache;
			 * the port has no bad_inode_ops, so drop it instead.
			 * i_nlink stays non-zero so eviction does not try to
			 * free the on-disk record.
			 */
...
{
	/*
	 * PORT: the kernel version works around a writeback race (I_SYNC)
	 * that the port's inode table does not have; unlinked inodes are
	 * torn down in ntfs_evict_big_inode().
	 */
...
{
	/* PORT: no inode_operations tables; api.c calls the vfs entry points
	 * directly. Only dir.c's file_operations are reached through i_fop. */
	if (S_ISDIR(mode)) {
...
static int ntfs_read_locked_attr_inode(struct inode *base_vi, struct inode *vi)
{
	...
	if (a->flags & (ATTR_COMPRESSION_MASK | ATTR_IS_SPARSE)) {
		if (a->flags & ATTR_COMPRESSION_MASK) {
			NInoSetCompressed(ni);
			ni->flags |= FILE_ATTR_COMPRESSED;
			/* PORT: named $DATA streams may be compressed too. */
			if (ni->type != AT_DATA) {
				ntfs_error(vi->i_sb,
					   "Found compressed non-data attribute.");
				goto unm_err_out;
			}
			if (vol->cluster_size > 4096) {
				ntfs_error(vi->i_sb,
...
int ntfs_read_inode_mount(struct inode *vi)
{
	...
	while (!(err = ntfs_attr_lookup(AT_DATA, NULL, 0, 0, next_vcn, NULL, 0,
		...
		if (!next_vcn) {
			...
			vi->i_mode = S_IFREG;
			/* PORT: no VFS initiated operations allowed for $MFT. */
			vi->i_fop = NULL;
		}
...
{
	/* PORT: no /proc/mounts; kept because super.c's sops references it. */
	return 0;
}
				 ...
				 const loff_t new_size, bool bsync)
{
	...
	if (!NInoCompressed(ni) && old_init_size < offset) {
		/* PORT: iomap_zero_range() -> run list based zeroing. */
		err = ntfs_vfs_zero_range(vi, old_init_size,
