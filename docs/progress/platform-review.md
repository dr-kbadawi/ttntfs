...
| 7 | `sync_inodes_sb()` wrote inodes in list order (newest first): `$MFT` and user records before `$Bitmap`. | Medium (crash ordering) | `test_inode.c: test_sync_order` | Stable partition: inodes 1..15, then ≥16, then inode 0 (newest first). COMMIT_CODE |
| 8 | `work.c`: the worker wrote `work->state` after `work->func()` returned. The kernel allows a work function to free its own `work_struct`. | Medium (UAF; `precalc_work` is embedded in `ntfs_volume` today, so latent) | `test_bdev.c: test_work` self-freeing items — ASan `heap-use-after-free in worker_main work.c:59` before the fix. | Running state kept in `wq->running`; the item is never touched after its function returns. COMMIT_CODE |
| 9 | `bdev_file.c`: sub-block writes on a raw device are a read-modify-write of the covering block with no serialisation: two threads updating different bytes of one block lost each other's bytes (4Kn disks: `i_blkbits`-sized folio tails are sub-block). | High on 4Kn devices | `test_bdev.c: test_rmw_race` (8 lanes in one 4 KiB block; every lane lost updates before the fix). Uses the additive test hook `ntfs_bdev_file_set_alignment()`. | Per-device `rmw_lock` around the RMW. COMMIT_CODE |
| 10 | `bdev_file.c` `file_flush`: an `EIO` from `F_FULLFSYNC` was masked by a succeeding `fsync()`. Also: `pread/pwrite` counts > `INT_MAX` fail on macOS; `/dev` nodes whose `DKIOCGETBLOCKCOUNT` fails got size 0 (every read `-EIO`). | Medium | Reviewed (no portable test for a failing `F_FULLFSYNC`). | Fall back to `fsync` only for `ENOTSUP/ENOTTY/EINVAL`; I/O chunked at 1 GiB; `lseek(SEEK_END)` size fallback. COMMIT_CODE |
| 11 | `asm/byteorder.h` `le*_to_cpup()` took typed pointers; UBSan flags misaligned pointers into packed on-disk structs (LZNT1). | Low (UBSan noise, UB by the letter) | Integrator report. | `const void *` parameters, memcpy-based. COMMIT_CODE |
| 12 | Every `folio_unlock()`/`folio_end_writeback()` took a global mutex and broadcast a global condvar. | Perf (rule 4: no global lock on the hot path) | `stress_pagecache` throughput. | Broadcast only when `state_waiters > 0` (Dekker-style seq_cst pair; no lost wake-up, see comment). COMMIT_CODE |
| 13 | `folio_put()` reaching zero for a folio still in the tree was silent. | Debug aid | — | `WARN_ON(folio->mapping)` on free; the lost-insert-race path clears `mapping` first. COMMIT_CODE |
