...
| 16 | `sync_inodes_sb()` -> `snapshot_inodes()` walked `sb->s_inodes` on every sync, the same O(cached) problem as finding 15 on the mapping side. | High (the other half of the delete degradation) | Same workload: with finding 15 alone, delete/s across 500 -> 8000 files was 900 -> 445. | `sb->s_dirty_inodes` + `inode->i_dirty_list`, same one-sided invariant. Added from `__mark_inode_dirty()` on the clean -> dirty edge after `i_lock` is dropped, and from the page cache when a mapping first dirties (`sync_inodes_sb` selects on mapping dirtiness too). Pruned only after `write_inode_now(sync=1)`, which has waited for that inode's writeback. Result: delete/s 960 -> 724 across the same range, i.e. close to flat. NB this is what Linux's VFS does with `wb->b_dirty`; only the driver is ported here, not the VFS, so the naive version was ours. |
