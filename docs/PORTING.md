...
| 4 | **`$LogFile` replay** -- our own module; no open implementation currently works | **Verified against Windows on four scenarios** (2026-09-17): file creation, directory index recovery, file extension, and chkdsk on a volume only we recovered. All match. Analysis always runs; replay stays opt-in per volume. Remaining: scenario B1 (hibernate mid-write) and a 512-byte-cluster stick for multi-cluster records. |
