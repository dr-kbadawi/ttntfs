...
| 4 | **`$LogFile` replay** -- our own module; no open implementation currently works | **Substantially advanced 2026-09-15.** Replay reproduced Windows' own result on a real Windows 10 dirty v2.0 journal: same recovered tree, file contents byte-identical, MFT record differing only in the per-write fixup slots, `ntfsck` clean. One scenario of five, so not yet closed. Analysis always runs; replay is opt-in per volume. |
